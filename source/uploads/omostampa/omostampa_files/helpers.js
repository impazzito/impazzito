...
function iframe_link(url, height) {
    ...
    inner_iframe += ' frameborder="0"'
    inner_iframe += ' width="100%"'
    inner_iframe += ' style="width:100%"'
    inner_iframe += ' height="'+height+'"'
    inner_iframe += ' src="'+iframe_href+'"'
    
    inner_iframe += '>Loading</iframe>'
    
    modal = default_popup(inner_iframe.toString(), title)
    
    modal.on('hidden', function() {
        var new_url = update_link_query_string(window.location.href);
        if (url.is('.update-on-close')) {
          start_ajax_loading();
          if (new_url == window.location.href) {
              window.location.reload();
          } else {
              change_window_location(new_url)
          }
        };
        if (url.is('.ajax-update-on-close')) {
          AjaxRequest({
              url:new_url
          });
        };
...
/* anchor utilis */
function get_ajax_location() {
    var location = this.location.href.split("#")[1];
    if (location && startswith(location, "!")) {
        return location.slice(1);
    }
    return 
}
function get_ajax_or_current_location() {
    var current = get_ajax_location();
    if (! current) current = window.location.pathname + window.location.search
    return current
}

function set_ajax_location(location, do_not_track) {
    //check if browser support html5 history push state
    
    if (typeof(window.history.pushState) == 'function') {
        window.history.pushState({url:location}, location, location);
    } else {
        window.location.hash = '!' + location;
    } 
    
    if (! do_not_track) { track_url(location) }
    
    return location;
}

function exec_after_json_call() {
    for (index in after_json_call) {
        after_json_call[index]();
    };
    jQuery(".tooltip.in,.popover.in").remove();
}
/* dom ready actions */
jQuery(document).ready(function() {
    
    jQuery('body').on('click', '.updatedlink', function(e) {
        var url = jQuery(this);
        url.attr("href", update_link_query_string(url.attr("href")));
    }).on("click", "a.updatenext", function(e) {
        var url = jQuery(this);
        url.attr("href", add_get_param(
            url.attr("href"), 
            "next", 
            get_ajax_or_current_location())
        );
    }).on("click", "."+DIALOG_CLASS, function(e) {
        if (e.which == 1) {
            e.preventDefault();
            iframe_link(this);
        }
        ...
        track_action("form-submit", form.attr("id"));
        start_ajax_loading();
        
        if (form.hasClass("updatenext")) {
            form.attr("action", add_get_param(
                form.attr("action"), 
                "next", 
                get_ajax_or_current_location())
            );
    ...
    // Fix placeholder problem for ie and old browsers
